...
suite "basic tests":
  ...
  test "sub two u8 vectors":
    var a, b: Vector[uint8]
    a.store(4'u8)
    b.store(2'u8)

    let c = a - b
    check(c.index(uint8, 0) == 2'u8)

  test "sub two u16 vectors":
    var a, b: Vector[uint16]
    a.store(256'u16)
    b.store(128'u16)

    let c = a - b
    check(c.index(uint16, 0) == 128'u16)

  test "sub two u32 vectors":
    var a, b: Vector[uint32]
    a.store(512'u32)
    b.store(256'u32)

    let c = a - b
    check(c.index(uint32, 0) == 256'u32)

  test "sub two u64 vectors":
    var a, b: Vector[uint64]
    a.store(1024'u64)
    b.store(512'u64)

    let c = a - b
    check(c.index(uint64, 0) == 512'u64)
